...
Run class-balanced kNN (k=30, the best from E3) across all rts and datasets.
Compare vs: scANVI, Euclidean, Mahal-pooled.

Key question: Does CB-kNN maintain its advantage at rts=20 and rts=50,
or does it converge to standard kNN?

Usage:
    python src/experimental/e9_cb_knn_sweep.py
"""
...
OUT_DIR = ROOT / "outputs" / "_experimental" / "e9_cb_knn_sweep"
OUT_DIR.mkdir(parents=True, exist_ok=True)

K_BEST = 30  # Best k from E3
...
def cb_knn_predict(
    ...
    """Class-balanced kNN: weight = 1 / (class_count * distance^2)."""
...
def standard_knn_predict(
    ...
    nbrs = NearestNeighbors(n_neighbors=k_eff, algorithm="ball_tree", n_jobs=-1)
    nbrs.fit(train_z)
    _, nn_idx_all = nbrs.kneighbors(test_z)
    preds = []
    for i in range(len(test_z)):
        counts = Counter(train_labels[nn_idx_all[i]])
...
def run_one(run_dir: Path, rare_class: str) -> dict | None:
    ...
    # CB-kNN (k=30)
    cb_pred = cb_knn_predict(ref_z, ref_labels, test_z, K_BEST, class_counts)
    cb_m, _ = classification_tables(y_true, pd.Series(cb_pred), rare_class=rare_class)

    # Standard kNN (k=30)
    std_pred = standard_knn_predict(ref_z, ref_labels, test_z, K_BEST)
    std_m, _ = classification_tables(y_true, pd.Series(std_pred), rare_class=rare_class)
    ...
    return {
        ...
        "std_knn_rare_f1":   std_m["rare_f1"],
        "cb_knn_rare_f1":    cb_m["rare_f1"],
    }
...
def main() -> pd.DataFrame:
        ...
        for seed in SEEDS:
            for rts in RTS_VALUES:
                ...
                print(f"  seed={seed} rts={rts:3d}  "
                      ...
                      f"StdKNN={result['std_knn_rare_f1']:.3f}  "
                      f"CB-kNN={result['cb_knn_rare_f1']:.3f}")
    ...
    for (dataset, rare_class, rts), grp in df.groupby(["dataset", "rare_class", "rts"]):
        for method, col in [
            ...
            ("Std-kNN",      "std_knn_rare_f1"),
            ("CB-kNN",       "cb_knn_rare_f1"),
        ]:
    ...
    print("\n\n=== E9 Summary: CB-kNN vs other methods (mean rare_f1) ===")
    ...
    # CB-kNN vs Euclidean delta
    df_cb  = agg[agg["method"] == "CB-kNN"][["dataset","rare_class","rts","mean_rare_f1"]].rename(columns={"mean_rare_f1":"cb_f1"})
    df_euc = agg[agg["method"] == "Euclidean"][["dataset","rare_class","rts","mean_rare_f1"]].rename(columns={"mean_rare_f1":"euc_f1"})
    df_std = agg[agg["method"] == "Std-kNN"][["dataset","rare_class","rts","mean_rare_f1"]].rename(columns={"mean_rare_f1":"std_f1"})
    delta = df_cb.merge(df_euc, on=["dataset","rare_class","rts"]).merge(df_std, on=["dataset","rare_class","rts"])
    delta["cb_vs_eucl"] = delta["cb_f1"] - delta["euc_f1"]
    delta["cb_vs_std"]  = delta["cb_f1"] - delta["std_f1"]
    write_table(delta, OUT_DIR / "delta_analysis.csv")

    print("\n\n=== E9 Delta: CB-kNN vs Euclidean ===")
    print(delta[["dataset","rare_class","rts","euc_f1","std_f1","cb_f1","cb_vs_eucl","cb_vs_std"]].to_string(
