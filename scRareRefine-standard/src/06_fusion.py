...
Fusion parameters (temperature, alpha_min, beta) are selected on validation only,
then applied to test (inductive constraint preserved).
...
Writes:
    outputs/{dataset}/{run_id}/fusion/
        validation_grid.csv    metrics for all (temperature, alpha_min, beta) combos on val
        best_params.csv        selected parameters
        test_results.csv       final fusion metrics on test
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Stage 6: prototype-scANVI fusion")
    ...
    scanvi_test = test_pred[[c for c in test_pred.columns if c.startswith("prob_")]].rename(columns=lambda c: c.removeprefix("prob_"))

    baseline_accuracy = float(
        classification_tables(val_pred["true_label"], val_pred["predicted_label"], rare_class=rare_class)[0]["overall_accuracy"]
    )
    ...
    print(f"Running fusion grid search ({len(_fusion_grid())} combos) ...")
    val_results = []
    for temperature, alpha_min, beta in _fusion_grid():
        proto_val = prototype_probabilities(
            ...
            reference_is_labeled=ref_is_labeled,
            temperature=temperature,
        )
        fused_val = fuse(scanvi_val, proto_val, margin=val_pred["margin"].to_numpy(), alpha_min=alpha_min, beta=beta)
        metrics = fusion_effect(val_pred["true_label"], val_pred["predicted_label"], fused_val, rare_class=rare_class)
    ...
    temperature, alpha_min, beta = select_best_params(
        val_results_df, baseline_accuracy=baseline_accuracy, max_false_rescue_rate=args.max_false_rescue_rate,
    )
    print(f"  Best params: temperature={temperature}, alpha_min={alpha_min}, beta={beta}")
    ...
    fused_test = fuse(scanvi_test, proto_test, margin=test_pred["margin"].to_numpy(), alpha_min=alpha_min, beta=beta)
    test_metrics = fusion_effect(test_pred["true_label"], test_pred["predicted_label"], fused_test, rare_class=rare_class)

    out_dir = run_dir / "fusion"
    write_table(val_results_df, out_dir / "validation_grid.csv")
    write_table(pd.DataFrame([{"temperature": temperature, "alpha_min": alpha_min, "beta": beta}]), out_dir / "best_params.csv")
    write_table(pd.DataFrame([test_metrics]), out_dir / "test_results.csv")

    print(f"  Test rare_f1={test_metrics['rare_f1']:.4f}  overall_accuracy={test_metrics['overall_accuracy']:.4f}")
