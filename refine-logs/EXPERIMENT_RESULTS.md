...
## M4：20-decision-seed 稳定性 — PASSED

- 覆盖 8 数据集 batch-heldout 中全部 15 个 `S<1.3` 单元，每单元 20 个确定性 decision seeds，共 300 次 OOF 审计；稳定性分析不加载 test 标签。
- 冻结规则原放行的 7 个单元均为 20/20 次放行；原拒绝的 8 个单元均为 0/20 次放行。
- 15/15 单元满足预登记的一致性标准，没有任何 pass rate 落入 0.2–0.8 不稳定区间。
- 原始重复行、单元汇总、报告及 PNG/PDF 图位于 `results/adaptive_separability_gate/v1/stability_20seeds/`。

## M4：6-human cell-stratified seed42 — PASSED

| Region | Variant | N | Mean F1 | Delta vs fixed | W/T/L vs fixed | Max FFR | Violations |
|---|---|---:|---:|---:|---:|---:|---:|
| ALL | fixed_s1.3 | 24 | 0.974522 | 0.000000 | 0/24/0 | 0.001870 | 0 |
| ALL | no_sep_gate | 24 | 0.973777 | -0.000745 | 1/22/1 | 0.003208 | 0 |
| ALL | adaptive_sep_gate | 24 | 0.974522 | 0.000000 | 0/24/0 | 0.001870 | 0 |

Cell-stratified 中 3 个低-S单元均来自 Baron pancreas，adaptive 全部安全拒绝并与 fixed 完全一致。结果说明在更容易、baseline F1 已较高的 split 下，自适应规则不会为追求表面增益而强行放宽 gate。

## 主线接入 — COMPLETED

- 原 `conformal_rescue()` 固定 `S=1.3` 行为保持不变；新增 `adaptive_conformal_rescue()` 与 `rescue_with_separability_gate()` dispatcher。
- 8 个主数据配置启用 `separability_gate_mode: adaptive`；命令行 `--separability_gate_mode fixed` 保留固定门槛对照。
- 新 `src` 实现与冻结实验实现对 15/15 个 batch-heldout 及 3/3 个 cell-stratified 低-S单元预测和关键审计字段完全一致。
- 定向相关测试 20 passed；完整回归 61 passed。
- 完成报告：`results/adaptive_separability_gate/v1/completion_report.md`。
