...
产出 2 张图（与 ablation.py 的两张表一一对应）：
  ablation_table1_components.{png,pdf}  表1·组件留一法：上 F1，下 incremental FPR max
  ablation_table2_rank.{png,pdf}        表2·rank 敏感性：上 F1，下 incremental FPR max
...
def plot_facets(df, variants, out_name, title, has_std=True):
    ...
    gs = fig.add_gridspec(
        2, n_ds, left=0.06, right=0.99, top=0.78, bottom=0.10, hspace=0.33, wspace=0.18
    )
    ...
    for col, (ds, label) in enumerate(DATASETS):
        ...
        bars = ax.bar(
            x, ffr, color=colors, edgecolor="black", linewidth=0.5, width=0.78
        )
        for i, h in enumerate(hi):
            if h:
                bars[i].set_edgecolor("#0D4A2A")
                bars[i].set_linewidth(1.6)
        # 标红越界 incremental FPR
        for i, v in enumerate(ffr):
            if not np.isnan(v) and v > INCREMENTAL_FPR_LIMIT:
                bars[i].set_edgecolor("#B04040")
                bars[i].set_linewidth(1.6)
        ax.axhline(INCREMENTAL_FPR_LIMIT, color="#B04040", ls="--", lw=1.0, zorder=0)
        ax.text(
            n_v - 0.3,
            INCREMENTAL_FPR_LIMIT + ffr_ymax * 0.02,
            f"α={INCREMENTAL_FPR_LIMIT}",
            color="#B04040",
            fontsize=8,
            ha="right",
            va="bottom",
        ...
        for i, v in enumerate(ffr):
            if not np.isnan(v):
                ...
                ax.text(
                    ...
                    fontsize=6.5,
                    fontweight="bold"
                    if (hi[i] or v > INCREMENTAL_FPR_LIMIT)
                    else "normal",
                    color="#B04040"
                    if v > INCREMENTAL_FPR_LIMIT
                    else ("#0D4A2A" if hi[i] else "#444444"),
                )
        ax.set_xticks(x)
        ax.set_xticklabels(tags, fontsize=8, rotation=20, ha="right")
        ax.set_ylim(0, ffr_ymax * 1.18)
        ax.grid(axis="y", alpha=0.3, linewidth=0.5)
        ax.set_axisbelow(True)
        ax.set_ylabel("incremental FPR\n(max over rts)" if col == 0 else "", fontsize=9)
        if col:
            ax.set_yticklabels([])
...
def main():
    ...
    plot_facets(
        t1,
        TABLE1,
        "ablation_table1_components",
        "Ablation Table 1 · component leave-one-out (6 datasets, 3 seeds × 4 rts).  "
        "Top: rare F1 (mean±SD).  Bottom: incremental FPR max (red dashed = α=0.01).  A5 full = ours.",
    )
    plot_facets(
        t2,
        TABLE2,
        "ablation_table2_rank",
        "Ablation Table 2 · candidate-rank sensitivity (6 datasets, 3 seeds × 4 rts).  "
        "adaptive selects per-val rank under incremental FPR≤α; fixed rank=3 exceeds the budget.  adaptive = ours.",
    )
