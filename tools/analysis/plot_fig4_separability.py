...
三联面板（PNG only）：
  ...
  (b) 合成 sep 扫描 — incremental FPR vs sep：full / nogate + α 线
  (c) 真实 benchmark — low_sep 阈值 vs mean F1(左) + worst-case incremental FPR(右)
...
ax.set_xlabel("separability_ratio")
ax.set_ylabel("rare-cell F1")
ax.set_ylim(-0.02, 1.02)
ax.set_title("(a) Synthetic sweep: F1")
ax.grid(True, ls="--", alpha=0.35)
ax.legend(loc="upper left")

# (b) 合成：incremental FPR vs sep
ax = axes[1]
ax.plot(x, sw.full_ffr, "o-", color=GREEN, lw=2, label="gate on")
ax.plot(x, sw.nogate_ffr, "s--", color=ORANGE, label="gate off")
ax.axhline(ALPHA, color=RED, ls="--", lw=1.1)
ax.text(x.max(), ALPHA + 0.0004, "α=0.01", color=RED, fontsize=8, ha="right")
ax.axvspan(x.min() - 0.05, LOW_SEP, color="#cccccc", alpha=0.18)
ax.axvline(LOW_SEP, color="k", ls="--", lw=1.1)
broke = sw[sw.nogate_ffr > ALPHA]
if len(broke):
    bs = float(broke.sep.max())
    ax.axvline(bs, color=RED, ls=":", lw=1.1)
    ax.text(
        bs + 0.03,
        ALPHA * 1.4,
        f"incremental FPR breaks\nonly at sep~{bs:.2f}",
        color=RED,
        fontsize=8,
        va="bottom",
    )
ax.set_xlabel("separability_ratio")
ax.set_ylabel("incremental FPR")
ax.set_title("(b) Synthetic sweep: incremental FPR")
ax.grid(True, ls="--", alpha=0.35)
...
ax = axes[2]
ax2 = ax.twinx()
xl = lsa.low_sep.to_numpy()
l1 = ax.plot(xl, lsa.f1_mean, "o-", color=GREEN, lw=2, label="mean rare F1 (L)")
l2 = ax2.plot(
    xl, lsa.ffr_max, "s--", color=ORANGE, lw=2, label="worst-case incremental FPR (R)"
)
ax2.axhline(ALPHA, color=RED, ls="--", lw=1.1)
ax2.text(1.62, ALPHA + 0.0004, "α=0.01", color=RED, fontsize=8, ha="right")
ax.axvline(LOW_SEP, color="k", ls="--", lw=1.0)
ax.text(LOW_SEP + 0.02, lsa.f1_mean.min(), "default 1.3", fontsize=8)
ax.set_xlabel("CONFORMAL_LOW_SEP (gate threshold)")
ax.set_ylabel("mean rare F1", color=GREEN)
ax2.set_ylabel("worst-case incremental FPR", color=ORANGE)
ax.set_title("(c) Real benchmark: low_sep sensitivity")
ax.grid(True, ls="--", alpha=0.3)
ll = l1 + l2
ax.legend(ll, [a.get_label() for a in ll], loc="center right")

fig.suptitle(
    "Separability gate (CONFORMAL_LOW_SEP=1.3): conservative, with a dataset-dependent risk axis.  "
    "(a,b) synthetic stress test on lung_endo: incremental FPR stays below budget well below 1.3.  "
    "(c) real benchmark: 1.3 is the smallest gate keeping worst-case incremental FPR ≤ α.",
    fontsize=9,
    y=1.04,
)
fig.tight_layout()
