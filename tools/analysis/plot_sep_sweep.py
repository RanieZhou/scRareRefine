...
两面板（x=实际 sep，因 t→sep 非单调，按 sep 排序）：
  ...
  右：FFR —— full / nogate，标 α=0.01 与 CONFORMAL_LOW_SEP=1.3
标出经验崩塌边界（nogate FFR 首次破 α）。
"""
...
# 经验崩塌点：nogate FFR 首次超过 α（按 sep 升序找最大的越界 sep）
...
ax1.plot(x, d["full_f1"], "o-", color="#2ca02c", lw=2, label="scRareRefine full (with sep gate)")
ax1.plot(x, d["nogate_f1"], "s--", color="#ff7f0e", label="rescue without sep gate")
ax1.axvline(LOW_SEP, color="k", ls="--", lw=1.2)
ax1.text(LOW_SEP + 0.02, 0.05, f"CONFORMAL_LOW_SEP={LOW_SEP}\n(full abstains to the left)", fontsize=8, va="bottom")
...
ax1.grid(True, ls="--", alpha=0.35); ax1.legend(fontsize=8, loc="upper left")
...
ax2.text(x.max(), ALPHA + 0.0004, "alpha = 0.01", color="#C0392B", fontsize=9, ha="right", va="bottom")
...
if break_sep:
    ...
    ax2.text(break_sep + 0.02, ALPHA * 1.3, f"empirical FFR-break\n~ sep {break_sep:.2f}", color="#C0392B", fontsize=8, va="bottom")
ax2.set_xlabel("separability_ratio (train-only)")
ax2.set_ylabel("False Rescue Rate (FFR)")
ax2.set_title("FFR vs separability")
ax2.grid(True, ls="--", alpha=0.35); ax2.legend(fontsize=8, loc="upper right")

fig.suptitle("Controlled separability sweep (lung_endo, semi-synthetic entanglement, seed=42, rts=0.05).\n"
             "Grey band = full method abstains (sep < 1.3).  Without the gate, rescue stays FFR-safe down to ~0.76 and breaks at ~0.69.",
             fontsize=10, y=1.02)
fig.tight_layout()
fig.savefig(OUT / "sep_sweep.png", bbox_inches="tight")
print(f"[saved] {OUT/'sep_sweep.png'}  | empirical FFR-break sep ~ {break_sep}")
