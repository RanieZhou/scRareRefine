"""scBalance baseline 对比实验 — 在 scanvi311 环境中运行。

在 HVG log1p 归一化表达上运行 scBalance（加权采样神经网络官方工具），
仅使用 is_labeled_for_scanvi 标注的训练子集作为参考集。

运行：
  D:/setup/anaconda/envs/scanvi311/python.exe tools/comparison/run_scbalance_comparison.py
"""
...
def main():
    ...
    for cfg_path, seed, rts_str in RUNS:
        ...
        train_lat = train_lat[
            [c for c in train_lat.columns if c.startswith("latent_")]
        ].to_numpy()
        ...
        if status == "ok":
            mres = _metrics(y_true, sb_pred, base_pred, rare_class)
            row.update(mres)
            print(
