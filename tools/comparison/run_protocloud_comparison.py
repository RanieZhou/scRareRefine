...
def make_run_dir(config: dict, seed: int, rts_str: str) -> Path:
    ...
    run_id = (
        f"{split_mode}_seed{seed}_{safe_class_name(rare_class)}_rare{_rts_label(size)}"
    )
...
def run_protocloud(config: dict, run_dir: Path, seed: int) -> np.ndarray:
    ...
    try:
        ...
        print(
            f"    HVG: {len(hvg_in_var)}/{len(hvg_genes)}  train_labeled: {len(labeled_ids)}  test: {len(test_ids)}"
        )

        def _get(cell_ids: list) -> tuple:
            rows = [id2row[cid] for cid in cell_ids if cid in id2row]
            return _load_counts(adata_hvg[rows], use_raw), [
                cid for cid in cell_ids if cid in id2row
            ]
        ...
        if (
            "IOSpec" in str(read_err)
            or "encoding_type" in str(read_err)
            or "null" in str(read_err).lower()
        ):
            print(
                f"    h5ad 格式不兼容 ({read_err.__class__.__name__})，通过 scanvi311 提取数据..."
            )
            train_X, train_ids_ok, test_X, test_ids_ok, hvg_in_var = (
                _extract_counts_via_subprocess(
                    config, hvg_genes, labeled_ids, test_ids, run_dir
                )
            )
            print(
                f"    HVG: {len(hvg_in_var)}/{len(hvg_genes)}  train_labeled: {len(train_ids_ok)}  test: {len(test_ids_ok)}"
            )
        else:
            raise
    ...
    pd.DataFrame({"cell_id": test_ids_ok, "pc_prediction": pc_pred}).to_csv(
        out_csv, index=False
    )
...
def main():
    ...
    for cfg_path, seed, rts_str in RUNS:
        ...
        if status == "ok":
            ...
            row.update(mres)
            print(
                f"  ProtoCloud: F1={mres['rare_f1']:.4f}  "
                ...
                agg_rows.append(
                    {
                        ...
                        "rec_mean": round(recs.mean(), 4),
                        "rec_std": round(recs.std(), 4),
    ...
    print("\n重绘 comparison_bars ...")
    subprocess.run(
        [SCANVI311_PYTHON, "tools/comparison/plot_comparison.py"], check=True
    )
