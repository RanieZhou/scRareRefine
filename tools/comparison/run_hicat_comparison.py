...
def run_hicat(config: dict, run_dir: Path, seed: int) -> np.ndarray:
    ...
    print(
        f"    HVG: {len(hvg_in_var)}/{len(hvg_genes)}  "
    ...
    if batch_key and (train_batch or test_batch):
        ...
        print(
            f"    Harmony(batch_key={batch_key}, n_batches={meta['batch'].nunique()})..."
        )
        ho = harmonypy.run_harmony(
            combined_pcs.astype(np.float64),
            meta,
            vars_use=["batch"],
            max_iter_harmony=10,
            random_state=seed,
    ...
    dbscan = DBSCAN()
    combined_dbscan = (
        dbscan.fit_predict(combined_umap).astype(np.float32).reshape(-1, 1)
    )
    ...
    pd.DataFrame({"cell_id": test_ids_ok, "hc_prediction": y_pred_final}).to_csv(
        out_csv, index=False
    )
