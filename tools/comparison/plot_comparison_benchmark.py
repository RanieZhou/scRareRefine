...
def _light_cmap(name, top=0.70, n=256):
    ...
    return LinearSegmentedColormap.from_list(
        f"{name}_light", base(np.linspace(0.0, top, n))
    )
...
METRICS = [
    ...
    ("rare_fp_rate", "(c) total target-class FPR", "Reds", False),
]
...
def plot_heatmap(df):
    ...
    for ax, (col, title, cmap, higher_better) in zip(axes, METRICS):
        ...
        # FPR 联越低越好且多数方法并列 0，故不打星，纯颜色 + colorbar 表达。
        if higher_better:
        ...
        ax.set_yticklabels(
            row_labels if ax is axes[0] else [""] * len(DATASETS), fontsize=8.5
        )
    ...
    fig.suptitle(
        ...
        "* = best method per dataset in (a) F1 and (b) recall.  In (c), darker = higher total target-class FPR.  "
...
def plot_boxplot(df):
    ...
    box_metrics = [
        ...
        ("rare_fp_rate", "total target-class FPR", True),
    ]
