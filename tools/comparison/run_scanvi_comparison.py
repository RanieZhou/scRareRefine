"""scANVI baseline 对比实验 — 在 scanvi311 环境中运行。

直接读取 Stage 2 (02_baseline_scanvi.py) 缓存的 scANVI test 预测，不重新预测，
只计算稀有类指标并写入共享对比表。是其余 4 个独立方法脚本（kNN/CellTypist/
scBalance/scRareRefine）的 base_pred 来源，也是 rescue_ffr 的参照基线。

运行：
  D:/setup/anaconda/envs/scanvi311/python.exe tools/comparison/run_scanvi_comparison.py
"""
...
def main():
    ...
    for cfg_path, seed, rts_str in RUNS:
        ...
        print(
            ...
            f"test_rare={int((y_true == rare_class).sum())}"
        )

        mres = _metrics(y_true, base_pred, base_pred, rare_class)
        print(
