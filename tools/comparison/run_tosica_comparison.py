...
def make_run_dir(config: dict, seed: int, rts_str: str) -> Path:
    ...
    run_id = (
        f"{exp.get('split_mode', 'batch_heldout')}_seed{seed}_"
...
def compute_row_metrics(y_true, pred, base_pred, rare_class: str) -> dict:
    ...
    n_false = int(
        ((pred != base_pred) & (pred == rare_class) & (y_true != rare_class)).sum()
    )
    return {
...
def run_tosica(
    cfg_path: str, config: dict, run_dir: Path, seed: int, rts_str: str
) -> np.ndarray:
    ...
    if cached_csv.exists():
        ...
        pred_arr = np.array(
            [id2pred.get(cid, base_pred[i]) for i, cid in enumerate(test_ids)]
        )
        return pred_arr
    ...
    try:
        ...
        (X_train, X_test, var_names, train_ids_ok, test_ids_ok, train_labels_ok) = (
            _extract_via_subprocess(
                cfg_path, config, run_dir, train_ids, train_labels, test_ids, hvg_genes
            )
        )

    print(
        f"    train: {len(train_ids_ok)} cells  test: {len(test_ids_ok)} cells  "
    ...
    ref_adata = ad.AnnData(
        X=X_train_norm, obs=obs_train, var=pd.DataFrame(index=var_names)
    )

    obs_test = pd.DataFrame(index=test_ids_ok)
    qry_adata = ad.AnnData(
        X=X_test_norm, obs=obs_test, var=pd.DataFrame(index=var_names)
    )
    ...
    if existing_weights:
        ...
        print(
            f"    训练 TOSICA (seed={seed}, epochs={TOSICA_EPOCHS}, max_gs={TOSICA_MAX_GS}) ..."
        )
        TOSICA.train(
            ref_adata,
    ...
    pred = np.array(
        [
            (
                id2tosica.get(cid, "Unknown")
                if id2tosica.get(cid, "Unknown") != "Unknown"
                else base_pred[i]
            )
    ...
    print(
        f"    TOSICA Unknown: {n_unknown}/{len(test_ids_ok)}  "
...
def main():
    ...
    run_key_set = {
        (load_config(c)["dataset"]["name"], str(int(s)), str(r)) for c, s, r in RUNS
    }
    ...
    for cfg_path, seed, rts_str in RUNS:
        ...
        if status == "ok":
            ...
            row.update(mres)
            print(
                f"  TOSICA: F1={mres['rare_f1']:.4f}  rec={mres['rare_recall']:.4f}  "
    ...
    for ds in ok["dataset"].unique():
        for rts in sorted(ok[ok["dataset"] == ds]["rare_train_size"].unique()):
            for method in ALL_METHODS:
                ...
                print(
                    f"  {ds:25s} rts={rts:4s}  {method:15s}: "
    ...
    subprocess.run(
        [SCANVI311_PYTHON, "tools/comparison/plot_comparison.py"], check=False
    )
