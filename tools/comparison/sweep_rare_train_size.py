...
def process_run(
    cfg_path: str, seed: int, rts_str: str, adata_cache: dict[str, ad.AnnData]
) -> list[dict]:
    ...
    if not cb._check_manifest(run_dir, config, seed, rts_str):
        print(f"[SKIP] {dataset} seed={seed} rts={rts_str} manifest 校验失败")
        return []
...
def main():
    ...
    out_dir = ROOT / "results" / "sweep_rts"
    ...
    for dataset in df["dataset"].unique():
        ...
        for method in METHODS:
            ...
            for rts in RTS_LIST:
                ...
                cells.append(
                    f"{r['f1_mean'].iloc[0]:.3f}±{r['f1_std'].iloc[0]:.3f}"
                    if len(r)
                    else "   -   "
                )
    ...
    for dataset in df["dataset"].unique():
        ...
        # lab_rare 映射
        lab_map = {
            rts: sub[sub["rare_train_size"] == rts]["lab_rare"].iloc[0]
            for rts in RTS_LIST
            if len(sub[sub["rare_train_size"] == rts])
        }
        md += [
            f"## {dataset}",
            "",
            "| 方法 | "
            + " | ".join(f"{r}({lab_map.get(r, '-')})" for r in RTS_LIST)
            + " |",
            "|------|" + "------|" * len(RTS_LIST),
        ]
        for method in METHODS:
