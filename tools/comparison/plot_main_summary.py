...
两面板（per-method 跨 6 数据集汇总；全 grid 明细见 Supp）：
  ...
  (b) worst-case FFR：各方法 rare_fp_rate 的全 benchmark 最大值 + α=0.01 红线
      —— показ：只有 scRareRefine 在拿到稀缺区高 F1 的同时把 worst-case FFR 压在 α 内。
...
def main():
    ...
    xlabels = [m + "†" if m == "HiCat" else ("scRareRefine\n(ours)" if m == "scRareRefine" else m) for m in names]
    ...
    # (b) per-method worst-case FFR（全 benchmark rare_fp_rate 最大）
    ffr_max = [df[df.method == m]["rare_fp_rate"].max() for m in names]
    ...
    fig.suptitle("scRareRefine: highest rare-cell F1 in the label-scarce regime while keeping worst-case FFR ≤ α.  "
                 "† HiCat is transductive (upper-bound reference).", fontsize=10, y=1.03)
    fig.tight_layout()
    OUT.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(OUT, bbox_inches="tight")
    print(f"[saved] {OUT}")
    # 控制台核对
    print("\nper-method 稀缺区 F1 均值 / worst-case FFR:")
    for m, fm, ff in zip(names, f1_mean, ffr_max):
        flag = "  <-- >α" if ff > ALPHA else ""
        print(f"  {m:14s} F1={fm:.3f}  FFRmax={ff:.4f}{flag}")
