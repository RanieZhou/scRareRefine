"""kNN baseline 对比实验 — 在 scanvi311 环境中运行。

在 scANVI latent embedding 上做欧氏 kNN，val 上 grid-search k∈{3,5,10,15}，
仅使用 is_labeled_for_scanvi 标注的训练子集作为参考集。

运行：
  D:/setup/anaconda/envs/scanvi311/python.exe tools/comparison/run_knn_comparison.py
"""
...
def main():
    ...
    for cfg_path, seed, rts_str in RUNS:
        ...
        val_true = val_pred["true_label"].astype(str).to_numpy()
        ...
        for k_cand in KNN_K_GRID:
            vp = _knn_predict(lab_lat, lab_labels, val_lat, k_cand)
            m, _ = classification_tables(val_true, vp, rare_class=rare_class)
            if m["rare_f1"] > best_val_f1:
                best_val_f1, best_k = m["rare_f1"], k_cand

        knn_pred = _knn_predict(lab_lat, lab_labels, test_lat, best_k)

        mres = _metrics(y_true, knn_pred, base_pred, rare_class)
        print(
            f"  {METHOD_NAME:15s}: F1={mres['rare_f1']:.4f}  rec={mres['rare_recall']:.4f}  "
            f"prec={mres['rare_precision']:.4f}  FP_rate={mres['rare_fp_rate']:.5f}  [k={best_k}]"
        )
        ...
        for rts in sorted(ok[ok["dataset"] == ds]["rare_train_size"].unique()):
            for method in ALL_METHODS:
                ...
                if sub.empty:
                    continue
                f1s, recs, fps = (
                    sub["rare_f1"].to_numpy(),
                    sub["rare_recall"].to_numpy(),
                    sub["rare_fp_rate"].to_numpy(),
                )
                agg_rows.append(
                    {
                        "dataset": ds,
